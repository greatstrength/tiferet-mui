...
This is the same org-level project shared with sibling repos (e.g. `tiferet-streamlit`, `tiferet-ly`). Field and option ids are unique per GitHub Project, not per repo, so these match the flagship's own binding.md. Re-resolve with `gh project field-list 2 --owner greatstrength --format json` if the project's fields ever change.
